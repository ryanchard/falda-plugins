...
If an argument was given, run `node "${CLAUDE_PLUGIN_ROOT}/hooks/login.mjs" finish '<argument>'` with Bash — single-quote the code in the command so shell metacharacters it might contain are never interpreted — and relay its output. On success tell the user to start a new Claude Code session so the MCP connection picks up the key. Never print `FALDA_TOKEN` or the settings file contents.
